function [egrad, w] = mygetPartialEuclideanGradientw(problem, x, I, storedb, key)
...
% Change log: 
    ...
    if isfield(problem, 'partialegradw')
    %% Compute the partial Euclidean gradient and Wu using partialegrade.
	
        % Check whether this function wants to deal with storedb or not.
        switch nargin(problem.partialegradw)
            case 2
                [egrad, w] = problem.partialegradw(x, I);
            case 3
                % Obtain, pass along, and save the store for x.
                store = storedb.getWithShared(key);
                [egrad, w, store] = problem.partialegradw(x, I, store);
                storedb.setWithShared(store, key);
            case 4
                % Pass along the whole storedb (by reference), with key.
                [egrad, w] = problem.partialegradw(x, I, storedb, key);
            otherwise
