function [egrad, w] = mygetEuclideanGradientw(problem, x, storedb, key)
...
%       Added support for getPartialEuclideanGradient
    ...
    %% Compute the Euclidean gradient using egrad.
        ...
        switch nargin(problem.egradw)
            case 1
                ...
                if ~isfield(store, 'egrad__') || ~isfield(store, 'w__')
                    [store.egrad__, store.w__] = problem.egradw(x);
    ...
    elseif canGetPartialEuclideanGradientw(problem)
    %% Compute the Euclidean gradient using a full partial Euclidean gradient.
        
        d = problem.ncostterms;
        [egrad, w] = mygetPartialEuclideanGradientw(problem, x, 1:d, storedb, key);
